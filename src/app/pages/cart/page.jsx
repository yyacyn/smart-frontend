...
export default function CartPage() {
    ...
    // 🔹 Fetch cart from backend on mount (requires Clerk token)
    useEffect(() => {
    ...
    // 🔹 Sync cart to backend whenever cartItems change (debounced in thunk)
    // Only upload if cart has been initialized (prevents overwriting DB with empty state on mount)
    useEffect(() => {
        if (!isSignedIn || !isInitialized) return;
        if (getToken) {
            dispatch(uploadCart({ getToken }));
        }
    }, [cartItems, dispatch, getToken, isSignedIn, isInitialized]);
    ...
    const toggleItemSelection = (id) => {
        setSelectedItems((prev) => {
            ...
            const updated = isSelected ? prev.filter((i) => i !== id) : [...prev, id];
            setSelectAll(updated.length === cartArray.length);
            return updated;
        });
    };
    ...
    const selectedItemsData = filteredCartItems.filter((item) =>
                ...
                {cartArray.length === 0 ? (
                    <div className="text-center py-16">
                        ...
                        <p className="text-gray-500 mb-6">
                            Ayo mulai berbelanja dan tambahkan produk ke keranjang!
                        </p>
                        <Link
                            href="/pages/marketplace"
                            className="btn bg-[#ED775A] border-none hover:bg-[#eb6b4b] text-white"
                        >
                            Mulai Belanja
                        </Link>
                    </div>
                    ...
                    <div className="grid gap-6 lg:grid-cols-[1fr_350px]">
                            ...
                            <div className="p-4 border-b border-gray-200">
                                <label className="flex items-center gap-3 cursor-pointer">
                                    ...
                                    <span className="font-medium text-gray-900">
                                        Pilih Semua ({cartArray.length} produk)
                                    </span>
                                </label>
                            </div>
                        ...
                        <div className="bg-white rounded-lg border-1 border-gray-200 p-6 h-fit sticky top-20">
                            ...
                            <button
                                className={`w-full btn mb-3 ${selectedItems.length === 0
                                    ? "bg-gray-400 text-white border-none cursor-not-allowed"
                                    : "bg-[#ED775A] border-none hover:bg-[#eb6b4b] shadow-none text-white"
                                    }`}
                                hidden={selectedItems.length === 0}
                                onClick={() => {
                                    if (selectedItems.length > 0) {
